...
namespace Adliance.AzureTools.DatabaseTools
{
    ...
    {
        private readonly ImportDatabaseParameters _parameters;

        public ImportDatabaseService(ImportDatabaseParameters parameters)
        {
        ...
        public async Task Run()
        {
            try
            {
                var sourcePath = FindSourceFileName(_parameters.Source);
                ...
                SqlPackageAdapter.ImportDatabase(_parameters.Target.Replace(targetDbName, temporaryDbName), sourcePath);
        ...
        private string FindSourceFileName(string source)
        {
            var noTargetNameProvided = string.IsNullOrEmpty(source);
            if (noTargetNameProvided)
            {
                throw new Exception("No backup file specified.");
            }
            if (!source.EndsWith(".bacpac", StringComparison.InvariantCultureIgnoreCase))
            {
                source = $"{source}.bacpac";
            }

            var sourceFileInfo = new FileInfo(source);
            if (!sourceFileInfo.Exists)
            {
                throw new Exception($"The specified backup \"{source}\" does not exist.");
            }
